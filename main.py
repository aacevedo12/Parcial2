...
@app.route('/consume')
def consume():
    return render_template('data.html', data=get_panama_vaccination_data(), cache_timeout=0)


def get_panama_vaccination_data():
    tree = ET.parse('templates/API_SH.IMM.MEAS_DS2_en_xml_v2_5381695.xml')
    tree = ET.parse('API_SH.IMM.MEAS_DS2_en_xml_v2_5381695.xml')
    tree = ET.parse('data/API_SH.IMM.MEAS_DS2_en_xml_v2_5381695.xml')
    root = tree.getroot()
    panama_data = []
